...
% clear; clc; close all;
function [] = test_covtype(para_p, delta, random_seed)
...
% for test\
nsamples = 300;%round(N*0.01);
% idx_sample = randsample(N, round(N*0.15));
idx1 = randsample(N_0, round(nsamples/2));
idx2 = randsample(N_1, round(nsamples/2)) + N_0;
...
folder = ['data/covtype/type', num2str(careclass(1)), num2str(careclass(2)),...
        '_rng', num2str(random_seed), '_bins', num2str(n_bins_), '_ww_out', num2str(-log10(dec_outloop_)),...
        '_in', num2str(-log10(err_inloop_)), '_n', num2str(nsamples),...
        '_maxin_', num2str(maxiter_inner_), '_maxout_', num2str(maxiter_outer_),'/'];
